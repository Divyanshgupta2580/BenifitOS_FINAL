    ...
    return (
      <main className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-6 text-center">
        <div className="bg-white p-8 rounded-2xl border border-slate-200 max-w-md w-full shadow-xs">
          <p className="text-sm font-semibold text-rose-600 mb-4">Could not load application details from server.</p>
      ...
      <main className="max-w-4xl mx-auto px-4 pt-6 space-y-6">
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-slate-500 pt-2 border-t border-slate-100">
            ...
            <div>
              <span className="font-semibold block text-slate-700">Submitted Date:</span>
              <span>{app.submittedAt ? new Date(app.submittedAt).toLocaleDateString() : 'Draft Mode'}</span>
            </div>
            <div>
              <span className="font-semibold block text-slate-700">Last Updated:</span>
        ...
        <Card>
          ...
          {app.attachedDocumentIds && app.attachedDocumentIds.length > 0 ? (
            ...
            <p className="text-xs text-slate-500 italic">No vault documents attached to this application.</p>
          )}
        </Card>

        {/* Verification Officer Remarks */}
        <Card>
          <h2 className="text-base font-bold text-blue-900 mb-2">Verification Officer Remarks</h2>
