    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-12">
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-xs">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-slate-500">Step {step} of 4: Direct Benefit Transfer</span>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 pt-6 space-y-6">
        ...
        <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
          <div className="bg-blue-900 h-full transition-all duration-300" style={{ width: `${(step / 4) * 100}%` }} />
        </div>
        ...
        {step === 1 && (
          <Card>
            <h2 className="text-lg font-bold text-blue-900 mb-1">Step 1: Select Target Scheme</h2>
            <p className="text-xs text-slate-500 mb-4">Choose the government scheme you wish to apply for.</p>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-xs font-bold font-mono text-amber-700">{s.code}</span>
        ...
        {step === 2 && (
          <Card>
            <h2 className="text-lg font-bold text-blue-900 mb-1">Step 2: Citizen Profile Auto-Fill</h2>
            <p className="text-xs text-slate-500 mb-4">Review information auto-populated from your verified profile.</p>

            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
              ...
              <div className="sm:col-span-2">
                <span className="text-xs text-slate-500 block">District & State Address</span>
                <span className="text-sm font-bold text-slate-900">
        ...
        {step === 3 && (
          <Card>
            <h2 className="text-lg font-bold text-blue-900 mb-1">Step 3: Attach Vault Documents</h2>
            <p className="text-xs text-slate-500 mb-4">Select verified documents from your Document Vault to link to this application.</p>
            ...
            <div className="space-y-3 mb-6">
              {documents.length > 0 ? (
                ...
                <p className="text-xs text-slate-500 italic">No documents in vault. You can still proceed and attach later.</p>
              )}
            </div>
        ...
        {step === 4 && (
          <Card>
            <h2 className="text-lg font-bold text-blue-900 mb-1">Step 4: Self-Declaration & Submission</h2>
            <p className="text-xs text-slate-500 mb-4">Review summary details before submitting to government welfare portal.</p>

            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-4 space-y-2">
              <div>
                <span className="text-[10px] text-slate-500 block uppercase font-bold">Selected Scheme</span>
                <span className="text-sm font-bold text-blue-900">{selectedScheme?.title || 'Scheme Selected'}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 block uppercase font-bold">Documents Attached</span>
                <span className="text-sm font-bold text-slate-900">{selectedDocIds.length} Vault Documents Linked</span>
              </div>
            </div>

            <label className="flex items-start gap-3 p-3 bg-blue-50/50 rounded-xl border border-blue-200 mb-6 cursor-pointer">
              <input
                type="checkbox"
                checked={declarationChecked}
                onChange={(e) => setDeclarationChecked(e.target.checked)}
                className="w-5 h-5 text-blue-900 rounded border-slate-300 focus:ring-blue-500 mt-0.5"
              />
